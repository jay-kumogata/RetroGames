...
# Pyxelの初期化
pyxel.init(64, 32, title="Amabie", fps=5)
...
pyxel.images[0].set(32, 0, [
    "00707000",  # squid-0 (plague-0)
    "00770770",
    "07707700",
    "00070700",
])
pyxel.images[0].set(40, 0, [
    "00070700",  # squid-1 (plague-1)
    "07707700",
    "00770770",
    "00707000",
])
pyxel.images[0].set(48, 0, [
    "70070707",  # boom (Enmaku)
    "00700070",
    "00707070",
    "07070700",
    "07000707",
...
class Game:
    ...
    def draw_title(self):
        ...
        #pyxel.text(16, 24, "Press SPACE to start", 7)

    def update_player(self):
        old_x = self.player_x
        if pyxel.btn(pyxel.KEY_A) or pyxel.btn(pyxel.KEY_LEFT):  # Aキーで左移動
            self.player_x -= 3
        if pyxel.btn(pyxel.KEY_D)or pyxel.btn(pyxel.KEY_RIGHT):  # Dキーで右移動
            self.player_x += 3
        if self.player_x < 0:
            self.player_x = 0
        if self.player_x > 56:
            self.player_x = 56
        if (pyxel.btnp(pyxel.KEY_S) or pyxel.btnp(pyxel.KEY_DOWN)) and self.bomb_state == 0:  # Sキーでボム発射
    ...
    def update_bomb(self):
        if self.bomb_state == 1:  # 落下中
            self.bomb_y += 1
            if self.bomb_y >= 25 or not (pyxel.btn(pyxel.KEY_S) or pyxel.btn(pyxel.KEY_DOWN)):
    ...
    def update_enemies(self):
        ...
        if self.enemy_timer > 30:
            self.spawn_enemy()
            self.enemy_timer = random.randint(10, 30)
    ...
    def draw(self):
        if self.state == "title":
            ...
            pyxel.cls(0)
            pyxel.blt(self.player_x, self.player_y, 0, 16, 0, 8, 5)  # お墓描画
            self.draw_waves()
    ...
    def update(self):
        if self.state == "title":
            ...
            if pyxel.btnp(pyxel.KEY_SPACE):  # スペースキーでリスタート
                self.state = "title"
